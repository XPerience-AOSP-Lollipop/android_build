...
android_team = “XPerience-AOSP-Lollipop“
...
def create_dependency_manifest(dependencies):
    ...
    if len(projects) > 0:
        os.system("repo sync --force-sync --force-broken --no-clone-bundle %s" % " ".join(projects))
...
def fetch_device(device):
    ...
    if not project is None:
        ...
        os.system('repo sync --force-sync --force-broken --no-clone-bundle %s' % device_dir)
