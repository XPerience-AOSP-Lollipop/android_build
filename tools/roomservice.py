...
if not depsonly:
    githubreq = urllib.request.Request("https://api.github.com/search/repositories?q=%s+user:XPerience-AOSP-Lollipop+in:name" % device)
    add_auth(githubreq)
    ...
    try:
        ...
        sys.exit()
    if (numresults == 0):
        print("Could not find device %s on github.com/XPerience-AOSP-Lollipop" % device)
        sys.exit()
